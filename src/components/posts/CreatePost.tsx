...
const CreatePost = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (bannedWordInTitle || bannedWordInContent) {
      ...
      alert(`Your post contains the forbidden word: "${bannedWord}". Please modify it IMMEDIATELY. 🚨`);
      setLoading(false);
      return;
    }
  ...
  const renderMediaPreview = () => {
    ...
    if (deezerLink.includes("deezer")) {
      ...
      if (match) {
        ...
        return (
          <iframe
            src={`https://www.deezer.com/plugins/player?format=square&autoplay=false&playlist=false&width=300&height=300&color=007FEB&layout=dark&size=medium&type=tracks&id=${deezerId}`}
            width="100%"
            height="80"
            frameBorder="0"
            allow="encrypted-media"
          ...
          <form onSubmit={handleSubmit} className="w-full max-w-md flex flex-col items-center">
            ...
            <div className="flex justify-end w-full gap-4">
              ...
              <button
                ...
                disabled={loading}
              >
                {loading ?     <svg aria-hidden="true" className="inline w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-purple-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-black p-6 rounded-xl w-full max-w-md">
      <h2 className="text-xl mb-4 text-white flex items-center">
        Enter Spotify 
        <FontAwesomeIcon icon={faSpotify} style={{ color: "#1DB954", fontSize: "24px" }} className="mx-2" />
        or Deezer
        <svg
    viewBox="0 0 24 24"
    width="24"
